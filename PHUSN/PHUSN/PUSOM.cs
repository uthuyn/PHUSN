...
public class PUSOM
{
    ...
    private void husspanFirstTimeForPUSOM(int[] prefix, int prefixLength, IList<QMatrixHUSSpan> database, float minPer, float maxPer, float minAvg, float maxAvg)
    {
            ...
            if (entry.Value >= minUtility)
            {
                ...
                if (checkPeriodicByIDList(seqIdList, maxPer))
                {
                    ...
                    if (totalUtility >= minUtility)
                    {
                        ...
                        //        items[i] = prefix[i];
                        //    }
                        //    System.Console.WriteLine("***non-periodic: " + arrayToString(items) + ":" + totalUtility.ToString());
                        //}

                    }
                    //else
                    //{
                    //    //Noted add to collect information
                    //    int[] _items = new int[1];
                    //    for (int i = 0; i < _items.Length; i++)
                    //    {
                    //        _items[i] = prefix[i];
                    //    }
                    //    System.Console.WriteLine("***non-HUSP: "+ arrayToString(_items) + ":" + totalUtility.ToString());
                    //    //MessageBox.Show(arrayToString(_items) + ":" + totalUtility.ToString());
                    ...
                    if (peut >= minUtility)
                    {
                        ...
                        {
                            husspanForPUSOM(prefix, 1, utilityChains, 1, minPer, maxPer, minAvg, maxAvg);
                ...
                //        _items[i] = prefix[i];
                //    }
                //    System.Console.WriteLine("@@@ Not satify 1-length pattern: "+ arrayToString(_items) + ":" + totalUtility.ToString());
                //}
            }
        }
    }

    private void husspanForPUSOM(int[] prefix, int prefixLength, UtilityChainList utilityChains, int itemCount, float minPer, float maxPer, float minAvg, float maxAvg)
    {
        // =======================  I-Extension  ===========================/
        ...
        foreach (KeyValuePair<int, float> entry in iList.SetOfKeyValuePairs())
        {
            ...
            if (entry.Value >= minUtility)
            {
                ...
                if (checkPeriodicByIDList(seqIdListIConcatenation, maxPer))
                {
                    ...
                    //{
                    //    //Noted add to collect information
                    //    int[] _items = new int[prefixLength + 1];
                    //    for (int i = 0; i < _items.Length; i++)
                    //    {
                    //        _items[i] = prefix[i];
                    //    }
                    ...
                    if (peut >= minUtility)
                    {
                        ...
                        {
                            husspanForPUSOM(prefix, prefixLength + 1, utilityChainsIConcatenation, itemCount + 1, minPer, maxPer, minAvg, maxAvg);
                        }
                    }
                }
            }
        ...
        foreach (KeyValuePair<int, float> entry in sList.SetOfKeyValuePairs())
        {
            ...
            if (entry.Value >= minUtility)
            {
                ...
                if (checkPeriodicByIDList(seqIdListSConcatenation, maxPer))
                {
                    ...
                    //{
                    //    //Noted add to collect information
                    //    int[] _items = new int[prefixLength + 2];
                    //    for (int i = 0; i < _items.Length; i++)
                    //    {
                    //        _items[i] = prefix[i];
                    //    }
                    ...
                    if (peut >= minUtility)
                    {
                        ...
                        if (itemCount + 1 < maxPatternLength)
                        {
                            husspanForPUSOM(prefix, prefixLength + 2, utilityChainsSConcatenation, itemCount + 1, minPer, maxPer, minAvg, maxAvg);
                        }
                    }
                }
            }
    ...
    public void runAlgorithmforPUSOM(string external, string input, float minUntil, string output, float minPer, float maxPer, float minAvg, float maxAvg)
    {
        ...
        husspanFirstTimeForPUSOM(patternBuffer, 0, database, minPer, maxPer, minAvg, maxAvg);
